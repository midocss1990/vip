...
public OnPluginStart()
{
	...
	SetMenuTitle(g_hColorsMenu, "Цвет дыма:\n \n");
}
...
public OnMapStart()
{
    ...
    {  
		...
		AddMenuItem(g_hColorsMenu, "", "Нет цветов", ITEMDRAW_DISABLED);  
    }
...
public Handler_ColorsMenu(Handle:hMenu, MenuAction:action, iClient, Item)
{
	switch(action)
	{
		...
		case MenuAction_Select:
		{
			...
			VIP_PrintToChatClient(iClient, "\x03Вы изменили цвет дыма на \x04%s", sColorName);
		...
		if (iEntity != -1)
		{
			...
			if(strcmp(sBuffer, "teamcolor") == 0)
			{
				switch (GetClientTeam(iClient))
				{
					case 2 :
					{
						FormatEx(sBuffer, sizeof(sBuffer), "200 25 25");
					}
					case 3 :
					{
						FormatEx(sBuffer, sizeof(sBuffer), "25 25 200");
					}
				}
			}
			else if(strcmp(sBuffer, "random") == 0)
			{
				FormatEx(sBuffer, sizeof(sBuffer), "%i %i %i", GetRandomInt(0, 255), GetRandomInt(0, 255), GetRandomInt(0, 255));
			}
