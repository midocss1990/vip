...
public OnMapStart()
{
	...
	SetMenuTitle(g_hColorsMenu, "Цвет трейлов гранат:\n \n");
...
public Handler_ColorsMenu(Handle:hMenu, MenuAction:action, iClient, Item)
{
	...
	//		LogToFile("addons/sourcemod/logs/VIP_GrenadeTrails.log", "Select -> %s, %s (%i)", sInfo, sColorName, Item);
			...
			VIP_PrintToChatClient(iClient, "\x03Вы изменили цвет трейлов гранат на \x04%s", sColorName);
...
UTIL_LoadColor(iClient, const String:sInfo[])
{
	...
	{
		GetClientTeamColor(iClient, GetClientTeam(iClient));
	}
	else
	{
...
public Action:Timer_OnSpawnProjectile(Handle:hTimer, any:iEntRef)
{
	...
	//	LogMessage("Timer_OnSpawnProjectile-> Entity: %i, iClient: %i", iEntity, iClient);
		...
		{
			TE_SetupBeamFollow(iEntity, g_iBeamSprite, 0, g_fLife, g_fStartWidth, g_fEndWidth, g_iFadeLength, g_iClientColor[iClient]);
			
			decl clients[MaxClients], i, totalClients;
			i = 1;
			totalClients = 0;
